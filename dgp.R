...
draw_nested_random_intercept <- function(tau, sigma_districts,sigma_schools, confounded = T, parametric_type, seed = seed){
  ...
  if(isTRUE(confounded)){
    ...
    # get average covs for each school
    asn <- dat %>%
      group_by(j) %>%
      summarise(z = mean(z)) %>%
      left_join(dat %>%
                  # remove all the vars im not using to assign z
    ...
    # create true p.score use -5 as intercept coeff 
